...
int main()
{
  ...
  spline_init();
  ReadCoor("Coords.xyz",Natoms,particle,Interlayer);
  read_file("CH.airebo");
    //Xmin = Xmax = 0.0;
  //Ymin = Ymax = 0.0;
  //Zmin = Zmax = 0.0;
  ...
  for(atomi=0 ; atomi < Natoms ; atomi++){
    ...
    NNeighb[atomi] = 0;
    NList[atomi]   = 0;
    //if(particle[atomi].r[2] == 0.0)particle[atomi].r[0]+=1.0;
    //else particle[atomi].layer=1;
  }
...
void calc_dE_REBO(point *F_REBO,double *angle,int *BondNum,int Natoms,point L,BondParamsStruct *BondParams,int *Bonded,atom *particle,double *Fc,int *NBond,double &E_REBO,int *NBond_once,int *Bonded_once)
{
  E_REBO=0.0;
#pragma omp parallel
  {
...
void relax_cell(int Natoms,int *NBond,AtomParamsStruct *AtomParams,BondParamsStruct *BondParams,int *Bonded,double *Bmat,double *qvec,double *Avec,double *rij,double *rijShiled_1,point L, atom *particle,Normal_struct *Normal,double *Fc,FILE* logfile,double *angle,int *BondNum,int *Neighb,int *NNeighb,int Coulomb_flag,int Interlayer,int *Normal_atom,int *NList,int *List,point *R0,int *NBond_once,int *Bonded_once){
  ...
  Calc_Potential(Natoms,NBond,Bonded,particle,BondParams,BondNum,rij,L,AtomParams,Bmat,qvec,Avec,rijShiled_1,logfile,angle,Normal,Normal_atom,Interlayer,Fc,Neighb,NNeighb,Coulomb_flag,origEnergy,NBond_once,Bonded_once);
  ...
  for(dir=0;dir<3;dir++){
    ...
    Calc_Potential(Natoms,NBond,Bonded,particle,BondParams,BondNum,rij,L,AtomParams,Bmat,qvec,Avec,rijShiled_1,logfile,angle,Normal,Normal_atom,Interlayer,Fc,Neighb,NNeighb,Coulomb_flag,newEnergy,NBond_once,Bonded_once);
...
#pragma omp for 
	...
	//Normal[atomi].z=Normal[atomj].z=1.0;
	//Normal[atomi].y=Normal[atomj].y=0.0;
	//Normal[atomi].x=Normal[atomj].x=0.0;
	//****
	dP_ij_drk=0.0;
	dP_ji_drk=0.0;
	Pij_term1=0.0;
...
#pragma omp critical
	ERep += exp_term1*(Epsilon_C_vdW_exp) - A_term;
	...
	dEvdW_drij = term3*dr_ij_drk + term4*dP_ij_drk + term5*dP_ji_drk + term6*dr_ij_drk;
	
	FvdW[atomk].x -= dEvdW_drij;
	FvdW[atomi].x += dEvdW_drij;
	//printf("FvdWxi=%.16f FvdWxk=%.16f  atomi=%i atomj=%i \n",FvdW[atomi].x,FvdW[atomk].x,atomi,atomk );
	//********dirivative in the y direction******
	...
	FvdW[atomk].y -= dEvdW_drij;
	FvdW[atomi].y += dEvdW_drij;
	//printf("FvdWyi=%.16f FvdWyk=%.16f  atomi=%i atomj=%i \n",FvdW[atomi].y,FvdW[atomk].y,atomi,atomk );
	//********dirivative in the z direction******
	...
	FvdW[atomk].z -= dEvdW_drij;
	FvdW[atomi].z += dEvdW_drij;
	//printf("FvdWzi=%.16f FvdWzk=%.16f  atomi=%i atomj=%i \n",FvdW[atomi].z,FvdW[atomk].z,atomi,atomk );
	...
	for (n=0;n<3;n++){
	  ...
	  if (fabs(inside_acos_ij) < 1.0){
	    ...
	    dangle = -1.0/sqrt_inside_ij*dinside_acos;
	    dP_ij_drk =  r_ij*cos_angle_ij*dangle;
	  }
	  ...
	  if (fabs(inside_acos_ji) < 1.0){
	    ...
	    dangle = -1.0/sqrt_inside_ji*dinside_acos;
	    dP_ji_drk = r_ij*cos_angle_ji*dangle;
	  }
	  
	  dEvdW_drij = ( term4*dP_ij_drk + term5*dP_ji_drk );
	  FvdW[atomk].x -= dEvdW_drij;
	  if(atomi==10 && atomj==4 && atomk==11){
	    //printf("FvdWxi=%.16f FvdWxk=%.16f  atomi=%i atomk=%i term4=%.16f dP_ij_drk=%.16f term5=%.16f dP_ji_drk=%.16f\n dN[0].r[0]=%.16f dN[0].r[1]=%.16f dN[0].r[2]=%.16f Normal[atomj].x=%.16f Normal[atomj].y=%.16f Normal[atomj].z=%.16f\n",FvdW[atomi].x,FvdW[atomk].x,atomi,atomk,term4,dP_ij_drk,term5,dP_ji_drk,dN[0].r[0],dN[0].r[1],dN[0].r[2],Normal[atomj].x,Normal[atomj].y,Normal[atomj].z );
	    //printf("j Normal_atom1=%i Normal_atom2=%i Normal_atom3=%i \n",Normal_atom[atomj*3+0],Normal_atom[atomj*3+1],Normal_atom[atomj*3+2]);
	    //printf("i Normal_atom1=%i Normal_atom2=%i Normal_atom3=%i \n",Normal_atom[atomi*3+0],Normal_atom[atomi*3+1],Normal_atom[atomi*3+2]);
	    //printf("k Normal_atom1=%i Normal_atom2=%i Normal_atom3=%i \n",Normal_atom[atomk*3+0],Normal_atom[atomk*3+1],Normal_atom[atomk*3+2]);
	  ...
	  dEvdW_drij = ( term4*dP_ij_drk + term5*dP_ji_drk );
	  FvdW[atomk].y -= dEvdW_drij;
	  //printf("FvdWyi=%.16f FvdWyk=%.16f  atomi=%i atomk=%i \n",FvdW[atomi].y,FvdW[atomk].y,atomi,atomk );	  	  
	  dir = 2;
	  dr_ij_drk = 0.0;
	  if (fabs(inside_acos_ij) < 1.0){
	    if (Interlayer == 1)
	  ...
	  //dP_ij_drk = calc_dP_ij_dr(atomi,atomj,atomk,dir,particle,Natoms,rijShiled_1,r_ij,Normal,L,Normal_atom,Interlayer,rx_ij,ry_ij,rz_ij,inside_acos_ij,angle_ij,sqrt_inside_ij,sin_angle_ij,cos_angle_ij,Pij_term1,0,dN);
	  //dP_ji_drk = calc_dP_ij_dr(atomj,atomi,atomk,dir,particle,Natoms,rijShiled_1,r_ij,Normal,L,Normal_atom,Interlayer,rx_ji,ry_ji,rz_ji,inside_acos_ji,angle_ji,sqrt_inside_ji,sin_angle_ji,cos_angle_ji,Pji_term1,0,dN);
	  
	  dEvdW_drij = ( term4*dP_ij_drk + term5*dP_ji_drk );
	  FvdW[atomk].z -= dEvdW_drij;
	  //printf("FvdWzi=%.16f FvdWzk=%.16f  atomi=%i atomk=%i \n",FvdW[atomi].z,FvdW[atomk].z,atomi,atomk );
	...
	for (n=0;n<3;n++){
	  ...
	  dEvdW_drij = ( term4*dP_ij_drk + term5*dP_ji_drk );
	  FvdW[atomk].x -= dEvdW_drij;
	  //printf("FvdWxi=%.16f FvdWxk=%.16f  atomi=%i atomk=%i \n",FvdW[atomi].x,FvdW[atomk].x,atomi,atomk );
	  dir = 1;
	  dr_ij_drk = 0.0;
	  if (fabs(inside_acos_ij) < 1.0){
	    if (Interlayer == 1)
...
void Calc_Potential(int Natoms, int *NBond, int *Bonded, atom *particle, BondParamsStruct *BondParams, int *BondNum, double *rij, point L, AtomParamsStruct *AtomParams, double *Bmat, double *qvec, double *Avec,double *rijShiled_1, FILE* logfile,double *angle,Normal_struct *Normal,int *Normal_atom,int Interlayer,double *Fc,int *Neighb,int *NNeighb,int Coulomb_flag,double &E_tot,int *NBond_once,int *Bonded_once)
{
  int atomi;
  double E_TORSION,E_KC,E_REBO;
  dr_ij *dN = new dr_ij[1];
  ...
  calc_dE_REBO_pot(angle,BondNum,Natoms,L,BondParams,Bonded,particle,Fc,NBond,E_REBO,NBond_once,Bonded_once);
  TORSION_pot(particle,BondNum,Natoms,L,Bonded,NBond,E_TORSION,NBond_once,Bonded_once);
  calc_F_Rep_pot(Natoms,particle,rijShiled_1,BondParams,L,Normal,Normal_atom,Interlayer,Neighb,NNeighb,dN,E_KC);
...
void calc_dE_REBO_pot(double *angle,int *BondNum,int Natoms,point L,BondParamsStruct *BondParams,int *Bonded,atom *particle,double *Fc,int *NBond,double &E_REBO,int *NBond_once,int *Bonded_once)
{
  E_REBO=0.0;
#pragma omp parallel
  {
...
void TORSION_pot(atom *particle,int *BondNum,int Natoms,point L,int *Bonded,int *NBond,double &E_TORSION,int *NBond_once,int *Bonded_once)
{
  E_TORSION=0;
#pragma omp parallel
  {
...
void calc_F_Rep_pot(int Natoms, atom *particle, double *rijShiled_1, BondParamsStruct *BondParams, point L,Normal_struct *Normal,int *Normal_atom,int Interlayer,int *Neighb,int *NNeighb,dr_ij *dN,double &ERep)
{
