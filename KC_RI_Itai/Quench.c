...
void Quench(int Natoms, int *NBond, int *Bonded, atom *particle, AtomParamsStruct *AtomParams, BondParamsStruct *BondParams,int *BondNum,double *rij, point &L, double dt, double tini, double tfin, double T, int NConserve, int NTraj, double *Bmat, double *qvec, double *dqvec, double *Avec, double *rijShiled_1, FILE* logfile, time_t &t0, clock_t &c0, double *angle, double OptTol,Normal_struct *Normal,int *Normal_atom,int Interlayer, double *Fc,double Xmin,double Ymin,double Zmin,double Xmax,double Ymax,double Zmax,int *Neighb,int *NNeighb,int Periodic_flag,int Coulomb_flag,int *NList,int *List,point *R0,int *NBond_once,int *Bonded_once)
{
  ...
  deltat= dt;
  count = 0;
  FILE *analysis = fopen("Results.dat","w");
  ...
  Calc_Potential(Natoms,NBond,Bonded,particle,BondParams,BondNum,rij,L,AtomParams,Bmat,qvec,Avec,rijShiled_1,logfile,angle,Normal,Normal_atom,Interlayer,Fc,Neighb,NNeighb,Coulomb_flag,E_REBO,NBond_once,Bonded_once,0,0,analysis);
...
#pragma omp parallel for 
  ...
  //Print_Traj(TrajFile,Natoms,0,particle);
  ...
  for(counter=1 ; (!converged) ; counter++){
	...
	calc_Normal(Natoms,Normal_atom,particle,Normal,L);
      ...
      Calc_Potential(Natoms,NBond,Bonded,particle,BondParams,BondNum,rij,L,AtomParams,Bmat,qvec,Avec,rijShiled_1,logfile,angle,Normal,Normal_atom,Interlayer,Fc,Neighb,NNeighb,Coulomb_flag,E_REBO,NBond_once,Bonded_once,0,counter,analysis);
    }
    //Enew =    E_tot;
    ...
    if(!(counter%NTraj)){
      ...
      Print_Traj(TrajFile,TrajFile_RI,TrajFile_Inter,Natoms,counter,particle,Xmin,Ymin,Zmin,Xmax,Ymax,Zmax,L);
      Print_Traj_gv(Natoms,particle,L);
      //Print_Traj(TrajFile,Natoms,counter,particle);
      //frame_counter++;
    }
  }
